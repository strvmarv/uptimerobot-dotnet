...
namespace UptimeRobotDotnet
{
    public partial class UptimeRobotClient
    {
        ...
        public async Task<MonitorsList> Monitors(MonitorSearchParameters parameters = null)
        {
            ...
            var result = await PostAsync<MonitorsList>(uri, parameters).ConfigureAwait(false);
            return result;
        }

        public async Task<MonitorsList> Monitor(MonitorSearchParameters parameters)
        {
            if (string.IsNullOrWhiteSpace(parameters.Api_Key)) parameters.Api_Key = _apiKey;
            var uri = new Uri($"{GetRelativePathWithVersion(MonitorsGetPath)}", UriKind.Relative);
            var result = await PostAsync<MonitorsList>(uri, parameters).ConfigureAwait(false);
        ...
        public async Task<Monitor> MonitorCreate(MonitorCreateParameters parameters)
        {
            if (string.IsNullOrWhiteSpace(parameters.Api_Key)) parameters.Api_Key = _apiKey;
            var uri = new Uri($"{GetRelativePathWithVersion(MonitorsCreatePath)}", UriKind.Relative);
            var result = await PostAsync<Monitor>(uri, parameters).ConfigureAwait(false);
            return result;
        }
        ...
        public async Task<Monitor> MonitorUpdate(MonitorUpdateParameters parameters)
        {
            if (string.IsNullOrWhiteSpace(parameters.Api_Key)) parameters.Api_Key = _apiKey;
